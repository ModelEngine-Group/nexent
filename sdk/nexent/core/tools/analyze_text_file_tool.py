...
class AnalyzeTextFileTool(Tool):
    ...
    def __init__(
        ...
        storage_client: Optional[MinIOStorageClient] = Field(
            description="Storage client for downloading files from S3 URLs. If None, only HTTP/HTTPS URLs will be supported.",
            default=None,
            exclude=True
        ),
        ...
        """Initialize the AnalyzeTextFileTool.
        
        Args:
            storage_client: Optional storage client for downloading files from S3 URLs.
                           If None, only HTTP/HTTPS URLs will be supported.
            observer: Message observer instance for real-time message transmission.
            data_process_service_url: URL of data process service.
    ...
    def _forward_impl(
        ...
        try:
            ...
            if self.observer:
                ...
                card_content = [{"icon": "file", "text": f"Analyzing file: {query}"}]
    ...
    def analyze_file(self, query: str, raw_text: str,):
        ...
        Process text file, convert to text using external API
        """
        language = getattr(self.observer, "lang", "en") if self.observer else "zh"
