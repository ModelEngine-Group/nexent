...
class KnowledgeBaseSearchTool(Tool):
    ...
    def __init__(
        ...
        # Retained for backward compatibility with older serialized tool configs.
        # Knowledge bases are globally readable at runtime, so this value is ignored.
        allowed_index_names: Optional[List[str]] = Field(
            description="Deprecated internal knowledge-base whitelist", default=None, exclude=True,
        ),
    ):
        ...
        _ = _unwrap_field_info(allowed_index_names)
    ...
    def forward(self, query: str, index_names: Optional[List[str]] = None) -> str:
        ...
        # Repository import may remove unavailable selections. Keep an empty
        # configuration explicit so the model receives a stable tool response.
        if not search_index_names:
            logger.warning(
                "KnowledgeBaseSearchTool called with query '%s' but no knowledge "
                "bases are configured",
                query,
            )
            return "No knowledge base is configured for this tool. Please update the agent configuration."
