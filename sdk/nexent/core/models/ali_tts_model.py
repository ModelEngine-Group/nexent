...
class AliTTSModel(BaseTTSModel):
    ...
    async def _generate_cosyvoice_streaming(self, text: str, ws_url: str, headers: Dict[str, str]) -> AsyncGenerator[bytes, None]:
    ...
    async def _generate_qwen_realtime_streaming(self, text: str, ws_url: str, headers: Dict[str, str]) -> AsyncGenerator[bytes, None]:
    ...
    def _log_error_diagnostics(self, error_msg: str) -> None:
        """Log diagnostic information for common errors."""
        if "418" in error_msg:
            logger.error(
                f"Voice compatibility error: voice={self.config.voice} may not be compatible with model={self.config.model}. "
                "CosyVoice models: Use longxiaochun_v2 (v2), longanyang (v3), or custom voices (v3.5). "
                "Qwen Realtime models: Use system voices like Cherry, or custom voices."
            )
        elif "1007" in error_msg:
            if self._is_realtime:
                logger.error("Protocol mismatch: URL contains /realtime but using CosyVoice protocol. Please check URL.")
            else:
                logger.error("Protocol mismatch: URL may be for Realtime API but using CosyVoice protocol.")
        elif "401" in error_msg or "403" in error_msg:
            logger.error("Authentication error: Check your API key is valid and has TTS permissions.")
        elif "task-started" in error_msg or "session.created" in error_msg:
            logger.error("Task/session initialization error: Check model name and URL are correct.")
