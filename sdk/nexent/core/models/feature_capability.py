...
FEATURE_CAPABILITY_OVERRIDE_SCHEMA_VERSION = 1
...
_OVERRIDE_CAPABILITY_FIELDS = {
    "reasoning": {
        "supported", "mode", "request_style", "efforts", "default_effort",
    },
    "prompt_cache": {"supported", "mode", "metrics_available"},
}
_OVERRIDE_POLICY_FIELDS = {
...
def normalize_feature_capability_override(
    override: Optional[Mapping[str, Any]],
) -> Optional[dict[str, Any]]:
    """Validate and sanitize one tenant-model feature override.

    Missing fields mean inheritance. Explicit ``None`` remains meaningful for
    tri-state capability facts, while unknown keys and inconsistent types are
    rejected instead of being silently persisted.
    """
    if override is None:
        return None
    if not isinstance(override, Mapping):
        raise ValueError("feature_capability_override must be an object or null")
    schema_version = override.get(
        "schema_version", FEATURE_CAPABILITY_OVERRIDE_SCHEMA_VERSION
    )
    if schema_version != FEATURE_CAPABILITY_OVERRIDE_SCHEMA_VERSION:
        raise ValueError("unsupported feature_capability_override schema_version")
    ...
    identity = override.get("authored_identity")
    if identity is not None:
        if not isinstance(identity, Mapping):
            raise ValueError("authored_identity must be an object")
        normalized["authored_identity"] = {
            key: str(identity.get(key) or "").strip()
            for key in ("model_factory", "model", "endpoint_host")
        }

    capability_patch = override.get("capability_patch", {})
    if not isinstance(capability_patch, Mapping):
        raise ValueError("capability_patch must be an object")
    for branch, allowed_fields in _OVERRIDE_CAPABILITY_FIELDS.items():
        value = capability_patch.get(branch)
        if value is None:
            continue
        if not isinstance(value, Mapping):
            raise ValueError(f"capability_patch.{branch} must be an object")
        unknown = set(value) - allowed_fields
        if unknown:
            raise ValueError(
                f"unsupported capability_patch.{branch} fields: {sorted(unknown)}"
            )
        branch_value = dict(value)
        if "supported" in branch_value and branch_value["supported"] not in (
            True,
            False,
            None,
        ):
            raise ValueError(f"capability_patch.{branch}.supported must be boolean or null")
        if branch == "reasoning":
            if "mode" in branch_value and branch_value["mode"] not in _VALID_REASONING_MODES:
                raise ValueError("unsupported reasoning mode")
            if (
                "request_style" in branch_value
                and branch_value["request_style"] not in _VALID_REQUEST_STYLES
            ):
                raise ValueError("unsupported reasoning request_style")
            if "efforts" in branch_value:
                efforts = branch_value["efforts"]
                if not isinstance(efforts, list) or not all(
                    isinstance(item, str) and item.strip() for item in efforts
                ):
                    raise ValueError("reasoning efforts must be a string array")
                branch_value["efforts"] = list(
                    dict.fromkeys(item.strip().lower() for item in efforts)
                )
            if "default_effort" in branch_value:
                effort = branch_value["default_effort"]
                if effort is not None and not isinstance(effort, str):
                    raise ValueError("reasoning default_effort must be a string or null")
                branch_value["default_effort"] = (
                    effort.strip().lower() if isinstance(effort, str) else None
                )
        else:
            if "mode" in branch_value and branch_value["mode"] not in _VALID_CACHE_MODES:
                raise ValueError("unsupported prompt_cache mode")
            if (
                "metrics_available" in branch_value
                and branch_value["metrics_available"] not in (True, False, None)
            ):
                raise ValueError(
                    "prompt_cache metrics_available must be boolean or null"
                )
        normalized["capability_patch"][branch] = branch_value

    policy = override.get("policy", {})
    if not isinstance(policy, Mapping):
        raise ValueError("policy must be an object")
    for branch, allowed_fields in _OVERRIDE_POLICY_FIELDS.items():
        value = policy.get(branch)
        if value is None:
            continue
        if not isinstance(value, Mapping):
            raise ValueError(f"policy.{branch} must be an object")
        unknown = set(value) - allowed_fields
        if unknown:
            raise ValueError(f"unsupported policy.{branch} fields: {sorted(unknown)}")
        branch_value = dict(value)
        if "enabled" in branch_value and not isinstance(branch_value["enabled"], bool):
            raise ValueError(f"policy.{branch}.enabled must be boolean")
        if branch == "reasoning" and "effort" in branch_value:
            effort = branch_value["effort"]
            if effort is not None and not isinstance(effort, str):
                raise ValueError("policy.reasoning.effort must be a string or null")
            branch_value["effort"] = (
                effort.strip().lower() if isinstance(effort, str) else None
            )
        normalized["policy"][branch] = branch_value

    return normalized


def _merge_capability_patch(
    baseline: Mapping[str, Any], override: Optional[Mapping[str, Any]]
) -> dict[str, Any]:
    profile = normalize_feature_profile(baseline) or normalize_feature_profile({})
    if not override:
        return profile
    merged = deepcopy(profile)
    for branch, patch in override.get("capability_patch", {}).items():
        merged[branch].update(deepcopy(patch))

    reasoning = merged["reasoning"]
    if reasoning["supported"] is not True:
        reasoning.update(
            {
                "mode": "none" if reasoning["supported"] is False else "unknown",
                "request_style": "none" if reasoning["supported"] is False else "unknown",
                "efforts": [],
                "default_effort": None,
            }
        )
    elif reasoning["mode"] == "always":
        reasoning.update(
            {"request_style": "none", "efforts": [], "default_effort": None}
        )
    elif reasoning["mode"] == "toggle":
        reasoning.update(
            {
                "request_style": "extra_body_enable_thinking",
                "efforts": [],
    ...
    elif reasoning["mode"] == "effort":
        if reasoning["request_style"] != "openai_reasoning_effort":
            raise ValueError("effort reasoning requires openai_reasoning_effort")
        if not reasoning["efforts"]:
            raise ValueError("effort reasoning requires at least one effort")
        if reasoning["default_effort"] not in reasoning["efforts"]:
            raise ValueError("reasoning default_effort must belong to efforts")
    else:
        raise ValueError("supported reasoning requires always, toggle, or effort mode")

    cache = merged["prompt_cache"]
    if cache["supported"] is not True:
        cache["mode"] = "none" if cache["supported"] is False else "unknown"
    elif cache["mode"] in {"none", "unknown"}:
        raise ValueError("supported prompt_cache requires a concrete mode")

    if override.get("capability_patch"):
        merged["source"] = "model_record_override"
        merged["match_kind"] = "model_record_override"
    return merged


def resolve_model_feature_configuration(
    baseline: Optional[Mapping[str, Any]],
    override: Optional[Mapping[str, Any]] = None,
    *,
    current_identity: Optional[Mapping[str, Any]] = None,
) -> dict[str, Any]:
    """Resolve persisted baseline and tenant-model override into runtime state."""
    normalized_override = normalize_feature_capability_override(override)
    capabilities = _merge_capability_patch(
        baseline or {}, normalized_override
    )
    policy_override = (
        normalized_override.get("policy", {}) if normalized_override else None
    )
    if policy_override:
        reasoning_policy = policy_override.get("reasoning", {})
        reasoning = capabilities["reasoning"]
        if reasoning_policy.get("enabled") is True and reasoning["supported"] is not True:
            raise ValueError("cannot enable unsupported or unknown reasoning")
        if reasoning_policy.get("enabled") is False and reasoning["mode"] == "always":
            raise ValueError("always-on reasoning cannot be disabled")
        requested_effort = reasoning_policy.get("effort")
        if requested_effort is not None and requested_effort not in reasoning["efforts"]:
            raise ValueError("reasoning effort override must belong to efforts")
        cache_policy = policy_override.get("prompt_cache", {})
        if (
            cache_policy.get("enabled") is True
            and capabilities["prompt_cache"]["supported"] is not True
        ):
            raise ValueError("cannot enable unsupported or unknown prompt_cache")
    policy = resolve_effective_feature_policy(
        capabilities,
        policy_override,
        source="model_record_override" if normalized_override else "nexent_default",
    )
    warnings = list(policy.get("warnings", []))
    authored_identity = (
        normalized_override.get("authored_identity") if normalized_override else None
    )
    if authored_identity and current_identity:
        identity_fields = ("model_factory", "model", "endpoint_host")
        if any(
            str(authored_identity.get(field) or "").strip().lower()
            != str(current_identity.get(field) or "").strip().lower()
            for field in identity_fields
        ):
            warnings.append("override_identity_mismatch")
    policy["warnings"] = list(dict.fromkeys(warnings))
    return {
        "baseline": normalize_feature_profile(baseline or {}),
...
def resolve_effective_feature_policy(
    ...
    *,
    ...
    return {
        ...
        "source": source or (
            "user_preference" if preference_map else "nexent_default"
        ),
        "warnings": warnings,
    }
