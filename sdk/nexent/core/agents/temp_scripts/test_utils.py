...
from nexent.core.agents.agent_model import (
    AgentRunInfo, AgentConfig, ModelConfig, AgentHistory, ToolConfig, SystemPromptComponent
)
...
def build_agent_run_info(
    ...
    # When ContextManager is enabled, the managed path delivers the system
    # prompt via context_components (SystemPromptComponent), mirroring
    # backend create_agent_info.py. assemble_final_context strips the leading
    # system message from the compressed history and re-prepends it only from
    # stable_messages (i.e. components); without a SystemPromptComponent the
    # rendered system_prompt (carried only as fallback_system_prompt) is
    # silently dropped from the model input.
    context_components = []
