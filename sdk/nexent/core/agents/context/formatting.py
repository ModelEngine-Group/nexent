...
def _format_skills_description(
    ...
    if language == "zh":
        ...
        lines.append("   **Workspace path contract (follow exactly)**:")
        lines.append("   - The code executor already runs inside `<run-workspace>/outputs`; create generated scripts with a bare filename such as `generated.js`, never `outputs/generated.js`.")
        lines.append("   - `run_skill_script(source=\"workspace\")` resolves from `<run-workspace>`; execute that same script as `outputs/generated.js`.")
        lines.append("   - `write_skill_file` edits an installed skill. Never use it to create a script or artifact for the current user request.")
        lines.append("   - Do not call a conversion or validation wrapper before its input exists, and never omit params required by the skill guide.")
        lines.append("   <code>")
        lines.append("   # Step 1 (execute this and wait for its result):")
        lines.append("   script = \"console.log('generate the requested artifact here')\"")
        lines.append("   with open(\"generated.js\", \"w\", encoding=\"utf-8\") as script_file:")
        lines.append("       script_file.write(script)")
        lines.append("   </code>")
        lines.append("   <code>")
        lines.append("   # Step 2 (only after step 1 succeeds):")
        lines.append("   result = run_skill_script(\"skill_name\", \"outputs/generated.js\", source=\"workspace\")")
        lines.append("   print(result)")
        lines.append("   </code>")
