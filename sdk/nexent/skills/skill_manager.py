...
class SkillManager:
    ...
    def get_tmp_file_path(self) -> Optional[str]:
        """Get the path to the temporary skill draft file.

        Returns:
            Absolute path to tmp.md, or None if local_skills_dir is not set
        """
        if not self.local_skills_dir:
            return None
        return os.path.join(self.local_skills_dir, "tmp.md")

    def tmp_file_exists(self) -> bool:
        """Check if tmp.md exists in the skills root directory.

        Returns:
            True if tmp.md exists
        """
        tmp_path = self.get_tmp_file_path()
        if not tmp_path:
            return False
        return os.path.exists(tmp_path)

    def read_tmp_file(self) -> Optional[str]:
        """Read the tmp.md file from skills root directory.

        Returns:
            Content of tmp.md, or None if not found or error
        """
        tmp_path = self.get_tmp_file_path()
        ...
        try:
            ...
            logger.error(f"Error reading tmp.md: {e}")
            return None

    def write_tmp_file(self, content: str) -> bool:
        """Write content to tmp.md in skills root directory.
        ...
        try:
            ...
            logger.error(f"Error writing tmp.md: {e}")
            return False

    def delete_tmp_file(self) -> bool:
        """Delete tmp.md from skills root directory.
        ...
        try:
            ...
            logger.error(f"Error deleting tmp.md: {e}")
            return False
