...
assert_file_contains "$PLAN_FILE" "to_regclass('nexent.conversation_message_t') IS NULL" "plan should detect a fresh database"
assert_file_contains "$PLAN_FILE" "\\if :nexent_schema_is_fresh" "plan should conditionally apply init SQL"
assert_file_contains "$PLAN_FILE" "\\i '$INIT_SQL_FILE'" "plan should apply init SQL for a fresh database"
assert_file_contains "$PLAN_FILE" "\\echo [sql-migrations] baseline __init.sql" "plan should baseline init SQL for an existing database"
assert_file_contains "$PLAN_FILE" "VALUES ('__init.sql'" "plan should record init SQL"
assert_file_contains "$PLAN_FILE" "\\set init_migration_status 'applied'" "plan should record fresh init SQL as applied"
assert_file_contains "$PLAN_FILE" "\\set init_migration_status 'baselined'" "plan should record existing init SQL as baselined"
assert_file_contains "$PLAN_FILE" ":'init_migration_status', 'v-test'" "plan should record init status and app version"
