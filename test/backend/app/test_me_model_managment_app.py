...
async def test_check_me_connectivity_failure():
    ...
    with patch('backend.apps.me_model_managment_app.check_me_connectivity_impl') as mock_connectivity:
        mock_connectivity.return_value = (
            "Connection failed, error code: 404",
            404,
            {
                "status": "Disconnected",
        ...
        assert response_data["data"]["connect_status"] == "unavailable"
...
async def test_check_me_connectivity_timeout():
    ...
    with patch('backend.apps.me_model_managment_app.check_me_connectivity_impl') as mock_connectivity:
        ...
        assert response.status_code == 200
        response_data = response.json()
        assert response_data["code"] == HTTPStatus.REQUEST_TIMEOUT
...
async def test_check_me_connectivity_exception():
    ...
    # Mock the check_me_connectivity_impl function in the app module
    with patch('backend.apps.me_model_managment_app.check_me_connectivity_impl') as mock_connectivity:
        mock_connectivity.side_effect = Exception("Test exception")
        ...
        assert response.status_code == 200
        response_data = response.json()
        assert response_data["code"] == HTTPStatus.INTERNAL_SERVER_ERROR
        assert "Connection failed" in response_data["message"]
...
async def test_check_me_model_healthcheck():
    ...
    assert response.json()[
        "data"]["connect_status"] == ModelConnectStatusEnum.AVAILABLE.value
