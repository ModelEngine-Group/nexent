...
@pytest.mark.asyncio
async def test_feature_override_create_requires_model_manager(
    client, auth_header, user_credentials, sample_model_data, mocker
):
    """P8-018: ordinary tenant members cannot author capability overrides."""
    mocker.patch(
        'backend.apps.model_managment_app.get_current_user_id',
        return_value=user_credentials,
    )
...
@pytest.mark.asyncio
async def test_feature_override_update_allows_model_manager(
    client, auth_header, user_credentials, mocker
):
    """P8-018: a tenant model manager can save a structured override."""
    mocker.patch(
        'backend.apps.model_managment_app.get_current_user_id',
        return_value=user_credentials,
    )
    ...
    response = await client.post(
        ...
        json={"feature_capability_override": override},
        headers=auth_header,
    )

    assert response.status_code == HTTPStatus.OK
    assert mock_update.await_args.args[3]["feature_capability_override"] == override
...
@pytest.mark.asyncio
async def test_manage_feature_override_requires_super_admin(
    client, auth_header, user_credentials, mocker
):
    """P8-018: cross-tenant capability changes are super-admin only."""
    mocker.patch(
        'backend.apps.model_managment_app.get_current_user_id',
        return_value=user_credentials,
    )
    ...
    payload = {
        ...
        "feature_capability_override": {
            "schema_version": 1,
            "capabilities": {"cache": {"supported": False}},
        },
    }

    response = await client.post(
        "/model/manage/create", json=payload, headers=auth_header
    )
