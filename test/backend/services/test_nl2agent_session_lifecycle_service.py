...
@pytest.mark.parametrize("status", ["active", "completed"])
def test_resolve_session_by_agent_restores_owned_configuration_session(
    monkeypatch, status
):
    lookup = MagicMock(return_value=_record(status=status))
    monkeypatch.setattr(lifecycle, "get_nl2agent_session", lookup)

    result = lifecycle.resolve_session_by_agent(
        draft_agent_id=202,
        tenant_id="tenant-a",
        user_id="user-a",
    )
...
def test_resolve_session_by_agent_hides_abandoned_session(monkeypatch):
    monkeypatch.setattr(
        lifecycle,
...
def test_cleanup_uses_configured_retention_and_batch(monkeypatch):
    ...
    assert lifecycle.cleanup_expired_sessions(now=now) == 4
