...
class TestRedisService(unittest.TestCase):
    ...
    def test_document_delete_fence_validation_and_redis_failures(self):
        service = RedisService()
        assert service.mark_document_delete_requested("idx") is False
        assert service.mark_document_delete_requested(
            "idx", "knowledge_base/a.txt", ttl_seconds=-1
        ) is False

        mock_client = MagicMock()
        mock_client.setex.side_effect = RuntimeError("set failed")
        service._client = mock_client
        assert service.mark_document_delete_requested("idx", "knowledge_base/a.txt") is False

        mock_client.delete.side_effect = RuntimeError("delete failed")
        assert service.clear_document_delete_marker("idx", "knowledge_base/a.txt") == 0
        mock_client.get.side_effect = RuntimeError("get failed")
        assert service.is_document_delete_requested("idx", "knowledge_base/a.txt") is False

    def test_document_delete_fence_clear_without_keys(self):
        service = RedisService()
        assert service.clear_document_delete_marker("idx") == 0
        assert service.is_document_delete_requested(None) is False

    @patch('backend.services.redis_service.REDIS_URL', 'redis://broker/0')
    @patch('backend.services.redis_service.REDIS_BACKEND_URL', 'redis://backend/1')
    def test_celery_control_app_is_lazy_and_rejects_missing_broker(self):
        service = RedisService()
        ...
        with patch('backend.services.redis_service.REDIS_URL', ''):
            ...
            with self.assertRaises(ValueError):
                missing._get_celery_control_app()

    def test_task_kwargs_normalization(self):
        assert RedisService._task_kwargs({"kwargs": '{"file_id": "f1"}'}) == {"file_id": "f1"}
        assert RedisService._task_kwargs({"kwargs": "bad-json"}) == {}
        assert RedisService._task_kwargs({"kwargs": ["not-a-dict"]}) == {}
        assert RedisService._task_kwargs({}) == {}

    def test_collect_runtime_task_ids_filters_matches_and_handles_inspector_failure(self):
        service = RedisService()
        inspector = MagicMock()
        inspector.active.return_value = {
            "worker": [
                {"id": "match-file", "kwargs": {"index_name": "idx", "file_id": "f1"}},
                {"id": "wrong-index", "kwargs": {"index_name": "other", "file_id": "f1"}},
                {"kwargs": {"index_name": "idx", "file_id": "f1"}},
                "not-a-task",
            ]
        }
        inspector.reserved.return_value = {
            "worker": [{"id": "match-path", "kwargs": {"index_name": "idx", "source": "a.txt"}}]
        }
        control_app = MagicMock()
        control_app.control.inspect.return_value = inspector
        service._get_celery_control_app = MagicMock(return_value=control_app)

        assert service._collect_runtime_task_ids("idx", "a.txt", "f1") == {"match-file", "match-path"}

        service._get_celery_control_app.side_effect = RuntimeError("inspect failed")
        assert service._collect_runtime_task_ids("idx", "a.txt", "f1") == set()

    def test_revoke_task_success_failure_and_empty_id(self):
        service = RedisService()
        assert service._revoke_task("") is False
        control_app = MagicMock()
        service._get_celery_control_app = MagicMock(return_value=control_app)
        assert service._revoke_task("task-1") is True
        control_app.control.revoke.assert_called_once_with("task-1", terminate=False)
        control_app.control.revoke.side_effect = RuntimeError("revoke failed")
        assert service._revoke_task("task-2") is False
    ...
    def test_prepare_document_deletion_keeps_zero_counts_when_cancel_or_revoke_fails(self):
        service = RedisService()
        service.mark_document_delete_requested = MagicMock(return_value=False)
        service._collect_runtime_task_ids = MagicMock(return_value={"task-1"})
        service.mark_task_cancelled = MagicMock(return_value=False)
        service._revoke_task = MagicMock(return_value=False)

        result = service.prepare_document_deletion("idx", "a.txt", file_id="f1")

        assert result["delete_fence_set"] is False
        assert result["runtime_tasks_found"] == 1
