...
def _patched_application(make_port, fake_stream, prepare_mock):
    ...
    with patch.object(application, "get_service", lambda: MagicMock()), \
         ...
         patch.object(application, "resolve_monitoring_user_email", return_value="user-1@example.com"), \
...
async def test_flush_if_due_uses_peek_then_take_without_transiently_empty_buffer():
    ...
    with _patched_application(make_port, fake_stream, prepare_mock) as application:
        # A normally exhausted stream must NOT leak StopAsyncIteration — it
        # used to escape the finally block and fail the claiming scheduler job.
    ...
    assert [chunk for batch in port._emits for chunk in batch] == [
        "chunk-0", "chunk-1", "chunk-2", "final",
    ]
    assert calls == [("finish", "completed")]
...
async def test_iterator_failure_flushes_buffer_before_failed_finish():
    """A consumed task exception must not interrupt the final buffer flush."""
    info = _FakeInfo()
    refs = {}
    emissions_at_finish = []
    ...
    def record_finish(port):
        original_finish = port.finish

        def finish(outcome):
            emissions_at_finish.append([chunk for batch in port._emits for chunk in batch])
            original_finish(outcome)

        port.finish = finish

    with pytest.raises(RuntimeError, match="iterator failed"):
        await _run_execute_attempt(fake_stream, info, port_hook=record_finish, refs=refs)

    assert emissions_at_finish == [["chunk-0"]]
    assert refs["finish_calls"] == ["failed"]
    assert not refs["port_ref"]["p"]._emit_lock.locked()
