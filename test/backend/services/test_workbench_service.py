...
def test_runtime_composer_mounts_request_scoped_children_without_mutating_root():
    root = AgentConfig(name="root", model_name="root-model", description="", tools=[])
    child = AgentConfig(name="child", model_name="child-model", description="", tools=[])
...
def test_generic_chat_resolves_tenant_system_agent_as_root(mocker):
    mocker.patch.object(
        workbench_service.system_agent_provider,
        "get_workbench_main_ref",
        return_value=SimpleNamespace(agent_id=99, version_no=4),
    ...
    lookup = mocker.patch.object(
        workbench_service,
        "search_agent_info_by_agent_id",
        return_value={"agent_id": 99, "name": "workbench_main", "enabled": True},
    )

    canonical, plan = workbench_service.resolve_workbench_config(
        WorkbenchSessionConfig(mode="generic_chat"), tenant_id="tenant"
    )

    assert canonical.agent_mounts == []
    assert plan.root.identity.agent_id == 99
    assert plan.root.identity.origin == "SYSTEM"
    assert plan.child_mounts == ()
    lookup.assert_called_once_with(99, "tenant", 4)


def test_multi_agent_chat_mounts_locked_agents_under_system_root(mocker):
    mocker.patch.object(workbench_service, "_get_user_role", return_value="ADMIN")
    mocker.patch.object(
        workbench_service.system_agent_provider,
        "get_workbench_main_ref",
        return_value=SimpleNamespace(agent_id=99, version_no=4),
    ...
    mocker.patch.object(
        ...
        side_effect=lambda agent_id, *_args: {7: 3, 8: 6}[agent_id],
    )
    snapshots = {
        99: {"agent_id": 99, "name": "workbench_main", "enabled": True},
        7: {"agent_id": 7, "name": "research", "enabled": True, "created_by": "user"},
        8: {"agent_id": 8, "name": "writer", "enabled": True, "created_by": "user"},
    }
    mocker.patch.object(
        workbench_service,
        "search_agent_info_by_agent_id",
        side_effect=lambda agent_id, *_args: snapshots[agent_id],
    )

    canonical, plan = workbench_service.resolve_workbench_config(
        WorkbenchSessionConfig(
            mode="multi_agent_chat",
            agent_mounts=[{"agent_id": 7}, {"agent_id": 8}],
        ),
        tenant_id="tenant",
        user_id="user",
    )

    assert [mount.version_no for mount in canonical.agent_mounts] == [3, 6]
    assert plan.root.identity.agent_id == 99
    assert [child.agent_id for child in plan.child_mounts] == [7, 8]
    assert [child.invocation_name for child in plan.child_mounts] == [
        "agent_7_v3",
        "agent_8_v6",
    ]
