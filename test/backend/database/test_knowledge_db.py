...
from backend.database.knowledge_db import (
    ...
    _generate_index_name, upsert_knowledge_record
)
...
def mock_session():
    ...
    # Make session.query() return mock_query regardless of arguments
...
def test_upsert_knowledge_record_create_new(monkeypatch, mock_session):
    """Test upsert_knowledge_record creates new record when none exists"""
    session, query = mock_session

    mock_record = MockKnowledgeRecord()
    mock_record.knowledge_id = 123
    mock_record.index_name = "new_kb"
    ...
    test_query = {
        "index_name": "new_kb",
        "knowledge_name": "New Knowledge Base",
        "knowledge_describe": "Test knowledge description",
...
def test_upsert_knowledge_record_update_existing(monkeypatch, mock_session):
    ...
    existing_record = MockKnowledgeRecord()
    existing_record.knowledge_id = 456
    existing_record.index_name = "existing_kb"
    existing_record.knowledge_name = "Old Name"
    existing_record.knowledge_describe = "Old description"
    existing_record.knowledge_sources = "elasticsearch"
    existing_record.embedding_model_name = "old_model"
    ...
    assert existing_record.knowledge_name == "Updated Knowledge Base"
    assert existing_record.knowledge_describe == "Updated description"
    assert existing_record.knowledge_sources == "datamate"
    assert existing_record.embedding_model_name == "new_model"
    assert existing_record.updated_by == "test_user"

    session.flush.assert_called_once()
    session.commit.assert_called_once()
    # Should not add new record when updating
