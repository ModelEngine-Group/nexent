...
def test_search_agent_info_by_agent_id_success(monkeypatch, mock_session):
    ...
    monkeypatch.setattr(
        "backend.database.agent_db.get_db_session", lambda: mock_ctx)
    monkeypatch.setattr("backend.database.agent_db.as_dict",
...
def test_create_agent_success(monkeypatch, mock_session):
    ...
    monkeypatch.setattr(
        "backend.database.agent_db.get_db_session", lambda: mock_ctx)
    monkeypatch.setattr(
        "backend.database.agent_db.filter_property", lambda data, model: data)
    monkeypatch.setattr("backend.database.agent_db.as_dict",
                        lambda obj: obj.__dict__)
    monkeypatch.setattr("backend.database.agent_db.AgentInfo",
...
def test_insert_related_agent_success(monkeypatch, mock_session):
    ...
    monkeypatch.setattr(
        "backend.database.agent_db.filter_property", lambda data, model: data)
    monkeypatch.setattr(
        "backend.database.agent_db.AgentRelation", lambda **kwargs: MagicMock())
...
def test_update_related_agents_add_new(monkeypatch, mock_session):
    ...
    monkeypatch.setattr(
        "backend.database.agent_db.AgentRelation", MockAgentRelationClass)
