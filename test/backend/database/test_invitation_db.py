...
from backend.database.invitation_db import (
    get_invitation_code_by_code,
    get_invitation_codes_by_tenant,
    create_invitation_code,
    update_invitation_code,
    soft_delete_invitation_code,
    get_invitation_records_by_invitation,
    create_invitation_record,
    get_invitation_records_by_user,
    get_invitation_usage_count,
    check_invitation_code_available
)
...
def test_get_invitation_code_by_code_success(monkeypatch, mock_session):
    ...
    result = get_invitation_code_by_code("test_code")
...
def test_get_invitation_code_by_code_not_found(monkeypatch, mock_session):
    ...
    result = get_invitation_code_by_code("nonexistent_code")

    assert result is None


def test_get_invitation_codes_by_tenant_success(monkeypatch, mock_session):
    ...
    result = get_invitation_codes_by_tenant("test_tenant")
...
def test_create_invitation_code_success(monkeypatch, mock_session):
...
def test_create_invitation_code_with_group_ids_list(monkeypatch, mock_session):
...
def test_update_invitation_code_success(monkeypatch, mock_session):
...
def test_soft_delete_invitation_code_success(monkeypatch, mock_session):
    ...
    result = soft_delete_invitation_code(invitation_id=123, updated_by="test_user")

    assert result is True


def test_get_invitation_records_by_invitation_success(monkeypatch, mock_session):
    ...
    result = get_invitation_records_by_invitation(123)
...
def test_create_invitation_record_success(monkeypatch, mock_session):
...
def test_get_invitation_usage_count_success(monkeypatch, mock_session):
    ...
    result = get_invitation_usage_count(123)

    assert result == 3


def test_check_invitation_code_available_success(monkeypatch, mock_session):
    """Test checking if invitation code is available"""
    session, query = mock_session

    mock_invitation = MockTenantInvitationCode()
    mock_invitation.invitation_id = 123
    mock_invitation.status = "IN_USE"
    mock_invitation.capacity = 5
    ...
    mock_ctx.__exit__.return_value = None
    monkeypatch.setattr("backend.database.invitation_db.get_db_session", lambda: mock_ctx)
    monkeypatch.setattr("backend.database.invitation_db.get_invitation_usage_count", lambda x: 2)

    result = check_invitation_code_available("test_code")

    assert result is True


def test_check_invitation_code_available_unavailable(monkeypatch, mock_session):
    """Test checking invitation code when it's unavailable"""
    session, query = mock_session
    ...
    mock_ctx.__exit__.return_value = None
    monkeypatch.setattr("backend.database.invitation_db.get_db_session", lambda: mock_ctx)
    monkeypatch.setattr("backend.database.invitation_db.get_invitation_usage_count", lambda x: 5)  # At capacity
...
def test_check_invitation_code_available_not_found(monkeypatch, mock_session):
    """Test checking invitation code when it doesn't exist"""
    session, query = mock_session
    ...
    result = check_invitation_code_available("nonexistent_code")
...
def test_database_error_handling(monkeypatch, mock_session):
    ...
    with pytest.raises(MockSQLAlchemyError, match="Database error"):
        get_invitation_code_by_code("test_code")
