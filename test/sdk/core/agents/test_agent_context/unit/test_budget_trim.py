...
from loader import trim_pairs_to_budget, trim_actions_to_budget, estimate_tokens_text
...
class TestBudgetTrimming:
    ...
    def test_trim_pairs_within_budget_returns_all(self):
        ...
        result = trim_pairs_to_budget(pairs, max_tokens=99999, render_fn=cm._renderer.pairs_to_text)
        assert len(result) == 3

    def test_trim_pairs_empty_input(self):
        cm = make_cm()
        assert trim_pairs_to_budget([], max_tokens=1000, render_fn=cm._renderer.pairs_to_text) == []
    ...
    def test_trim_pairs_keeps_at_least_last_when_all_overflow(self):
        ...
        result = trim_pairs_to_budget(pairs, max_tokens=1, render_fn=cm._renderer.pairs_to_text, keep_first=False)
        assert len(result) == 1
    ...
    def test_trim_pairs_keep_first_true_keeps_first_pair(self):
        ...
        first_pair_tokens = estimate_tokens_text(cm._renderer.pairs_to_text([pairs[0]]))
        result = trim_pairs_to_budget(pairs, max_tokens=first_pair_tokens + 5, render_fn=cm._renderer.pairs_to_text, keep_first=True)
    ...
    def test_trim_actions_within_budget_returns_all(self):
        ...
        result = trim_actions_to_budget(actions, task_text="", max_tokens=99999, render_fn=cm._renderer.actions_to_text)
        assert len(result) == 3

    def test_trim_actions_empty_returns_empty(self):
        cm = make_cm()
        assert trim_actions_to_budget([], task_text="", max_tokens=1000, render_fn=cm._renderer.actions_to_text) == []
    ...
    def test_trim_actions_keeps_last_when_overflow(self):
        ...
        result = trim_actions_to_budget(actions, task_text="", max_tokens=1, render_fn=cm._renderer.actions_to_text)
        assert len(result) >= 1
    ...
    def test_trim_actions_skips_drop_that_splits_tool_call_and_observation(self):
        ...
        two_act_tokens = estimate_tokens_text(cm._renderer.actions_to_text(actions[1:]))
        three_act_tokens = estimate_tokens_text(cm._renderer.actions_to_text(actions))
        max_tokens = two_act_tokens + (three_act_tokens - two_act_tokens) // 2

        result = trim_actions_to_budget(actions, task_text="", max_tokens=max_tokens, render_fn=cm._renderer.actions_to_text)
