...
def _register_stub_packages():
    ...
    # Stub for nexent.monitor (used by manager.py for OTel instrumentation)
    if "nexent.monitor" not in sys.modules:
        _monitor_stub = ModuleType("nexent.monitor")
        
        class _MockMonitoringManager:
            def is_enabled(self):
                return False
            ...
            def trace_llm_request(self, *args, **kwargs):
                from contextlib import contextmanager
                @contextmanager
                def _noop():
                    yield None
            ...
            def set_openinference_output(self, *args, **kwargs):
                pass
            
            def add_span_event(self, *args, **kwargs):
                pass
        
        _monitor_stub.get_monitoring_manager = lambda: _MockMonitoringManager()
        _monitor_stub.OPENINFERENCE_SPAN_KIND_CHAIN = "CHAIN"
        _monitor_stub.OPENINFERENCE_SPAN_KIND_LLM = "LLM"
        _monitor_stub.OPENINFERENCE_INPUT_VALUE = "input.value"
        _monitor_stub.OPENINFERENCE_OUTPUT_VALUE = "output.value"
        sys.modules["nexent.monitor"] = _monitor_stub
