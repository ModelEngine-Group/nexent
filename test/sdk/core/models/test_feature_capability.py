...
def test_p8_015_model_override_is_normalized_without_copying_baseline():
    override = normalize_feature_capability_override(
        {
            "capability_patch": {"reasoning": {"supported": False}},
...
def test_p8_016_admin_override_can_expand_unknown_capability():
    baseline = resolve_feature_capabilities("unknown", "private-model")
    result = resolve_model_feature_configuration(
        baseline,
        {
            "capability_patch": {
                "reasoning": {
            ...
            "policy": {"reasoning": {"enabled": True}},
        },
    )
    assert result["effective_capabilities"]["reasoning"]["supported"] is True
    assert result["effective_policy"]["reasoning"]["enabled"] is True
    assert result["effective_policy"]["source"] == "model_record_override"


def test_p8_016_invalid_override_combination_is_rejected():
    baseline = _profile()
    with pytest.raises(ValueError, match="at least one effort"):
        resolve_model_feature_configuration(
            baseline,
            {
                "capability_patch": {
...
def test_p8_017_override_survives_identity_change_with_warning():
    result = resolve_model_feature_configuration(
        _profile(),
        {
            "authored_identity": {
    ...
    assert result["effective_policy"]["prompt_cache"]["enabled"] is False
    assert result["warnings"] == ["override_identity_mismatch"]


def test_p8_017_null_override_restores_new_catalog_baseline():
    old = _profile(version="catalog@1", cache=False)
    new = _profile(version="catalog@2", cache=True)
    overridden = resolve_model_feature_configuration(
        old, {"policy": {"reasoning": {"enabled": False}}}
    )
    assert overridden["effective_policy"]["reasoning"]["enabled"] is False
    restored = resolve_model_feature_configuration(new, None)
    assert restored["effective_policy"]["reasoning"]["enabled"] is True
    assert restored["effective_policy"]["prompt_cache"]["enabled"] is True
