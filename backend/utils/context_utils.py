...
def _build_a2ui_card_guide_text(language: str = "zh") -> str:
    ...
    if language == "zh":
        ...
        lines.append("")
        lines.append("【核心规则】")
        lines.append("当需要展示卡片、表单、收集反馈或请求用户操作时，你必须调用 output_card 工具。")
        lines.append("这是生成交互式界面的唯一方式，不要使用其他方法。")
        lines.append("")
        lines.append("【绝对禁止】")
        lines.append("❌ 不要编写 Python 代码来构建卡片（如 from a2ui_components import ...）")
        lines.append("❌ 不要输出 JSON 格式的卡片定义（如 {\"type\": \"card\", ...}）")
        lines.append("❌ 不要在回复中包含 HTML 或 XML 标签")
        lines.append("❌ 不要尝试导入任何 UI 组件库")
        lines.append("")
        lines.append("【正确做法】")
        lines.append("✅ 直接调用 output_card 工具，传入参数即可生成卡片")
        lines.append("✅ 工具调用后简短回复「已生成卡片」即可")
        lines.append("")
        lines.append("【调用示例】")
        lines.append('# 展示信息卡片')
        lines.append('output_card(card_type="info", title="任务完成", message="数据处理成功")')
        lines.append('')
        lines.append('# 收集用户反馈')
        lines.append('output_card(card_type="feedback", title="满意度调查", message="您对服务满意吗？", options=["非常满意", "满意", "不满意"])')
        lines.append('')
        lines.append('# 请求用户确认')
        lines.append('output_card(card_type="confirmation", title="确认删除", message="确定要删除吗？", options=["确认", "取消"])')
        lines.append('')
        lines.append('# 收集用户信息')
        lines.append('output_card(card_type="form", title="信息收集", fields=[{"name": "name", "label": "姓名", "type": "textfield"}, {"name": "email", "label": "邮箱", "type": "textfield"}])')
        lines.append('')
        lines.append('# 请求用户评分')
        lines.append('output_card(card_type="rating", title="服务评分", message="请为本次服务打分")')
    else:
        lines = ["### Interactive Card Output (output_card tool)"]
        lines.append("")
        lines.append("[CORE RULE]")
        lines.append("When you need to display cards, forms, collect feedback, or request user actions, you MUST call the output_card tool.")
        lines.append("This is the ONLY way to generate interactive interfaces - do not use any other method.")
        lines.append("")
        lines.append("[ABSOLUTELY PROHIBITED]")
        lines.append("❌ Do NOT write Python code to build cards (e.g. from a2ui_components import ...)")
        lines.append("❌ Do NOT output JSON card definitions (e.g. {\"type\": \"card\", ...})")
        lines.append("❌ Do NOT include HTML or XML tags in your response")
        lines.append("❌ Do NOT attempt to import any UI component libraries")
        lines.append("")
        lines.append("[CORRECT APPROACH]")
        lines.append("✅ Call the output_card tool directly with parameters to generate cards")
        lines.append("✅ After calling the tool, reply briefly with 'Card generated'")
        lines.append("")
        lines.append("[EXAMPLES]")
        lines.append('# Display info card')
        lines.append('output_card(card_type="info", title="Task Complete", message="Data processed successfully")')
        lines.append('')
        lines.append('# Collect user feedback')
        lines.append('output_card(card_type="feedback", title="Satisfaction Survey", message="Are you satisfied?", options=["Very satisfied", "Satisfied", "Not satisfied"])')
        lines.append('')
        lines.append('# Request user confirmation')
        lines.append('output_card(card_type="confirmation", title="Confirm Delete", message="Are you sure?", options=["Confirm", "Cancel"])')
        lines.append('')
        lines.append('# Collect user information')
        lines.append('output_card(card_type="form", title="Information Collection", fields=[{"name": "name", "label": "Name", "type": "textfield"}, {"name": "email", "label": "Email", "type": "textfield"}])')
        lines.append('')
        lines.append('# Request user rating')
        lines.append('output_card(card_type="rating", title="Service Rating", message="Please rate this service")')
