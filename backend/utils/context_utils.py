...
def _build_execution_flow_text(
    ...
    if language == "zh":
        ...
        lines.append("  - 使用检索结果中的事实时，必须在相关句子或段落末尾紧跟对应引用标记；同一句可标记多个来源")
        ...
        lines.append("   - When stating facts from retrieval results, place the matching reference mark immediately at the end of the relevant sentence or paragraph; multiple sources may be marked on one sentence")
