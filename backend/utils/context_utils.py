...
def _build_execution_flow_text(
    ...
    if language == "zh":
        ...
        lines.append("  - 【必须执行】只要最终回答使用了任一检索结果中的事实，就必须在相关句子或段落末尾紧跟对应引用标记；同一句可标记多个来源")
        ...
        lines.append("   - **Mandatory**: whenever the final answer states a fact from a retrieval result, place the matching reference mark immediately at the end of that sentence or paragraph; multiple sources may be marked on one sentence")
