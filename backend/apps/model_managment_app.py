...
async def create_model(request: ModelRequest, authorization: Optional[str] = Header(None)):
    ...
    """
    try:
        if "feature_capability_override" in request.model_fields_set:
    ...
    except ModelFeatureConfigurationError as e:
        logging.warning("Invalid model feature configuration: %s", e)
        raise HTTPException(status_code=HTTPStatus.BAD_REQUEST, detail=str(e))
...
async def update_single_model(
    ...
    """
    try:
        if "feature_capability_override" in request:
            user_id, tenant_id = _require_model_manager(authorization)
        else:
            user_id, tenant_id = get_current_user_id(authorization)
