...
def _list_bundle_files() -> List[str]:
    ...
    try:
        ...
        for root, directories, files in os.walk(OFFICIAL_AGENTS_PATH):
            ...
            relative_root = os.path.relpath(root, OFFICIAL_AGENTS_PATH).replace("\\", "/")
            if "agent.json" in files and relative_root != ".":
                names.add(relative_root)
            for filename in files:
                if filename.lower() == "agent.json":
                    continue
                if filename.lower().endswith(".json") and filename[:-5]:
                    relative_name = os.path.join(relative_root, filename[:-5]).replace("\\", "/")
                    names.add(relative_name.lstrip("./"))
                elif filename.lower().endswith(".zip") and filename[:-4]:
                    relative_name = os.path.join(relative_root, filename[:-4]).replace("\\", "/")
                    names.add(relative_name.lstrip("./"))
        return sorted(names)
    except OSError as e:
...
def _load_bundle(name: str) -> Optional[OfficialAgentBundle]:
    ...
    if (
        ...
        logger.warning("Skip official agent bundle with unsafe name: %r", name)
        return None
    ...
    if os.path.isfile(zip_path):
        ...
        try:
            with zipfile.ZipFile(zip_path) as archive:
                ...
                for member in archive.infolist():
                    ...
                    if not (target == root or target.startswith(root + os.sep)):
                        raise ValueError("official agent ZIP contains unsafe path")
                    if member.is_dir():
            ...
            with open(os.path.join(extract_dir, "agent.json"), encoding="utf-8") as f:
                data = json.load(f)
            bundle = OfficialAgentBundle.model_validate(data)
            bundle.name = name
            _attach_skills_from_dir(bundle, extract_dir)
            _attach_kb_docs_from_dir(bundle, extract_dir)
            return bundle
        except (OSError, json.JSONDecodeError, ValueError, zipfile.BadZipFile) as e:
            logger.warning("Skip invalid official agent ZIP '%s': %s", name, e)
