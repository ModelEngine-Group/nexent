...
def _enrich_and_refresh_feature_configuration(
    record: Dict[str, Any],
) -> Dict[str, Any]:
    enriched = enrich_feature_configuration(record)
    if (
        enriched.pop("feature_capability_baseline_changed", False)
        and record.get("model_id") is not None
    ):
        update_model_record(
            record["model_id"],
            {"feature_capability_metadata": enriched["feature_capability_metadata"]},
        )
    return enriched
...
async def create_model_for_tenant(
    ...
    *,
    ...
    try:
        ...
        model_data["feature_capability_metadata"] = current_feature_baseline(
            model_data
        )
        if "feature_capability_override" in explicit:
            model_data["feature_capability_override"] = prepare_feature_override(
                model_data.get("feature_capability_override"), model_data
            )
...
async def update_single_model_for_tenant(
    ...
    *,
    ...
    try:
        ...
        explicit = set(explicit_fields or model_data.keys())
        feature_identity_changed = bool(
            explicit.intersection({"model_name", "model_repo", "base_url", "model_factory"})
        )
        ...
        feature_record = dict(existing_models[0])
        feature_record.update(model_data)
        if feature_identity_changed:
            feature_record["feature_capability_metadata"] = None
        refreshed_baseline = current_feature_baseline(feature_record)
        existing_feature_baseline = normalize_feature_profile(
            existing_models[0].get("feature_capability_metadata")
        )
        if (
            feature_identity_changed
            or "feature_capability_override" in explicit
            or (
                existing_feature_baseline is not None
                and existing_feature_baseline != refreshed_baseline
            )
        ):
            model_data["feature_capability_metadata"] = refreshed_baseline
        if "feature_capability_override" in explicit:
            model_data["feature_capability_override"] = prepare_feature_override(
                model_data.get("feature_capability_override"), feature_record
            )
...
async def list_models_for_tenant(tenant_id: str):
    ...
    try:
        ...
        for record in records:
            record = _enrich_and_refresh_feature_configuration(record)
