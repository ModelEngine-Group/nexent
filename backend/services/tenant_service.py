...
from consts.const import TENANT_NAME, DEFAULT_GROUP_ID
...
def get_tenant_info(tenant_id: str) -> Dict[str, Any]:
    ...
    if not name_config:
        logging.error(f"The name of tenant_id {tenant_id} not found.")
...
def create_tenant(tenant_id: str, tenant_name: str, created_by: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Raises:
        ValidationError: When tenant already exists or creation fails
    """
...
def _create_default_group_for_tenant(tenant_id: str, created_by: Optional[str] = None) -> int:
    ...
    try:
        ...
        logger.info(f"Created default group '{default_group_name}' (ID: {group_id}) for tenant {tenant_id}")
        return group_id
