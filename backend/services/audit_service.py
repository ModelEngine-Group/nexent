"""Security audit logging for successful sensitive operations.

Every entry is a single line prefixed with ``[SEC_AUDIT]`` and emitted through
the ``audit.security`` logger, which propagates to the root logger and lands in
the serving process' per-category log file (e.g. ``logs/config/nexent_config.log``
for the config service) without introducing a new log category::

    [SEC_AUDIT] event=user_signin result=success user_id=u1 tenant_id=t1 email=a@b.com ip=1.2.3.4 ua="Mozilla/5.0" session_id=- details={}

Only successful operations are recorded: a failing operation is treated as an
interception and stays in the existing business error logs. Recording is
best-effort: ``record_security_event`` never raises, so an audit failure can
never break the underlying business flow. Secrets (passwords, access keys,
tokens) must never be passed in; call sites only provide identifiers and
...
def record_security_event(
    ...
    """Record one security audit entry for a successful operation. Never raises."""
    try:
