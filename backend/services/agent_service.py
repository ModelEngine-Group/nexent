...
async def check_agent_name_conflict_batch_impl(
    ...
    for item in request.items:
        ...
        for agent in agents_cache:
            ...
            matches_display = (
                item.display_name and agent.get(
...
async def regenerate_agent_name_batch_impl(
    ...
    for item in request.items:
        ...
        results.append(
            {"name": agent_name, "display_name": agent_display_name})
...
def save_messages(
    ...
    if target == MESSAGE_ROLE["USER"]:
        ...
        submit(save_conversation_assistant, agent_request,
               messages, user_id, tenant_id)
