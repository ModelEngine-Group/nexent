...
# Configure logging with the shared LOG_LEVEL/IS_DEBUG settings.
configure_logging()
...
class ServiceManager:
    ...
    def start_workers(self):
        """Start isolated Celery workers for all data-processing queues."""
...
try:
    ...
    sys.exit(1)
                ...
                logger.info(
                    "Starting %s worker for queue=%s concurrency=%s",
                    config["name"],
                    config["queue"],
                    config["concurrency"],
                )
                ...
                def log_worker_output(process, worker_name):
                    ...
                    try:
                        ...
                        logger.warning(f"Error in log thread for worker {worker_name}: {e!s}")
                    finally:
            ...
            logger.error(f"❌ Error starting workers: {e!s}")
            import traceback
            logger.error(traceback.format_exc())
        ...
        try:
            ...
            def log_flower_output():
                ...
                try:
                    ...
                    logger.warning(f"❌ Error in Flower log thread: {e!s}")
                finally:
            ...
            logger.error(f"❌ Error starting Flower: {e!s}")
            import traceback
            logger.error(traceback.format_exc())
            return False
    ...
    def stop_all_services(self):
        ...
        if service_processes['workers']:
            ...
            for worker_info in service_processes['workers']:
                ...
                try:
                    ...
                    logger.error(f"Error stopping {name} worker: {e!s}")
        ...
        if service_processes['flower']:
            ...
            try:
                ...
                logger.error(f"An unexpected error occurred while stopping Flower: {e!s}")
...
def main():
    ...
    try:
        ...
        logger.error(f"Error starting service: {e!s}")
        sys.exit(1)
    finally:
        if service_manager and not service_manager._shutdown_called:
            service_manager.stop_all_services()
