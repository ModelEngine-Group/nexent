...
def delete_nl2agent_session_by_agent(
    *, tenant_id: str, user_id: str, draft_agent_id: int
) -> bool:
    """Soft-delete one owned session and its hidden conversation history."""
    with get_db_session() as session:
        record = (
            session.query(Nl2AgentSession)
            .filter(
...
def list_active_nl2agent_draft_ids(tenant_id: str, user_id: str) -> List[int]:
    """Return every active Draft id owned by one user for the Agent selector."""
    with get_db_session() as session:
        records = (
            session.query(Nl2AgentSession.draft_agent_id)
            .filter(
                Nl2AgentSession.tenant_id == tenant_id,
                Nl2AgentSession.user_id == user_id,
