...
def mask_access_key(access_key: Optional[str]) -> Optional[str]:
    """Mask an access key for list responses, keeping a short prefix/suffix.

    Only the create/refresh responses may return the full secret once; every
    listing must mask it so a tenant-wide read cannot harvest user keys.
    """
    if not access_key:
        return access_key
    if len(access_key) <= 8:
