...
async def create_tool_config_list(
    ...
    for tool in tools_list:
        ...
        if tool_config.class_name == "KnowledgeBaseSearchTool":
            ...
            if not index_names:
                # Keep the tool available when repository import removed every unavailable KB.
                logger.warning(
                    "Keeping knowledge_base_search tool for agent '%s' with no configured "
                    "knowledge bases for user '%s'.",
