...
class Nl2AgentFinalizeRequest(BaseModel):
    """Request body for finalizing a draft agent generated by NL2AGENT.

    The frontend FinalizeCard posts the full agent spec produced by the
    ``nl2agent_finalize_proposal`` skill here. Fields not provided fall back
    to ``None`` so the service can apply sensible defaults.
    """
    ...
    # LLM models
    business_logic_model_id: Optional[int] = Field(default=None, ge=1)
    ...
    # Prompts
    duty_prompt: Optional[str] = Field(default=None, max_length=8000)
...
class AgentRepositoryListingCreateRequest(BaseModel):
    ...
    tool_count: Optional[int] = Field(None, ge=0, description="Total tool count across all agents in the bundle")
