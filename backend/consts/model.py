...
class BatchTaskRequest(BaseModel):
    sources: List[Dict[str, Any]] = Field(
        ..., description="List of source objects to process"
    )
...
class HybridSearchRequest(BaseModel):
    ...
    index_names: List[str] = Field(
        ..., min_items=1, description="List of index names to search"
    )
    top_k: int = Field(10, ge=1, le=100, description="Number of results to return")
    weight_accurate: float = Field(
        0.5, ge=0.0, le=1.0, description="Weight applied to accurate search scores"
    )
...
class GroupCreateRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Tenant ID where the group belongs"
    )
...
class GroupMembersUpdateRequest(BaseModel):
    ...
    user_ids: List[str] = Field(
        ..., description="List of user IDs to set as group members"
    )
...
class SetDefaultGroupRequest(BaseModel):
    ...
    default_group_id: int = Field(
        ..., ge=1, description="Group ID to set as default for the tenant"
    )
...
class ManageTenantModelListRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to query models for"
    )
...
class ManageTenantModelCreateRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to create model for"
    )
    model_repo: Optional[str] = Field("", description="Model repository path")
    model_name: str = Field(..., description="Model name")
    model_type: str = Field(
        ..., description="Model type (e.g., 'llm', 'embedding', 'vlm', 'tts', 'stt')"
    )
    ...
    expected_chunk_size: Optional[int] = Field(
        None, description="Expected chunk size for embedding models"
    )
    maximum_chunk_size: Optional[int] = Field(
        None, description="Maximum chunk size for embedding models"
    )
...
class ManageTenantModelUpdateRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to update model for"
    )
    current_display_name: str = Field(
        ..., description="Current display name of the model to update"
    )
    ...
    display_name: Optional[str] = Field(
        None, description="New display name for the model"
    )
    expected_chunk_size: Optional[int] = Field(
        None, description="Expected chunk size for embedding models"
    )
...
class ManageTenantModelDeleteRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to delete model from"
    )
...
class ManageTenantModelHealthcheckRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to check model connectivity"
    )
...
class ManageBatchCreateModelsRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to batch create models for"
    )
    provider: str = Field(
        ..., description="Model provider (e.g., 'silicon', 'modelengine')"
    )
    type: str = Field(..., description="Model type (e.g., 'llm', 'embedding')")
    api_key: str = Field("", description="API key for the models")
    models: List[Dict[str, Any]] = Field(
        default_factory=list, description="List of models to create/update"
    )
...
class ManageProviderModelListRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to query provider models for"
    )
    provider: str = Field(
        ..., description="Model provider (e.g., 'silicon', 'modelengine')"
    )
...
class ManageProviderModelCreateRequest(BaseModel):
    ...
    tenant_id: str = Field(
        ..., min_length=1, description="Target tenant ID to create provider models for"
    )
    provider: str = Field(
        ..., description="Model provider (e.g., 'silicon', 'modelengine')"
    )
...
class VersionPublishRequest(BaseModel):
    ...
    version_name: Optional[str] = Field(
        None, description="User-defined version name for display"
    )
    release_note: Optional[str] = Field(
        None, description="Release notes / publish remarks"
    )
...
class VersionListItemResponse(BaseModel):
    ...
    source_version_no: Optional[int] = Field(
        None, description="Source version number if rollback"
    )
    source_type: Optional[str] = Field(
        None, description="Source type: NORMAL / ROLLBACK"
    )
    status: str = Field(
        ..., description="Version status: RELEASED / DISABLED / ARCHIVED"
    )
...
class VersionDetailResponse(BaseModel):
    ...
    tool_instances: List[dict] = Field(
        default_factory=list, description="Tool instance snapshots"
    )
    relations: List[dict] = Field(
        default_factory=list, description="Relation snapshots"
    )
...
class VersionRollbackRequest(BaseModel):
    ...
    version_name: Optional[str] = Field(
        None, description="New version name for the rollback version"
    )
    release_note: Optional[str] = Field(
        None, description="Release notes for the rollback version"
    )
...
class VersionUpdateRequest(BaseModel):
    ...
    release_note: Optional[str] = Field(
        None, description="Release notes / version description"
    )
