...
def _build_security_headers(agent: dict) -> dict:
    """Build auth headers from the agent's securitySchemes + security_credentials."""
    ...
    for req in requirements:
        for scheme_id in (req.get("schemes", {}) if isinstance(req, dict) else {}):
            credential = credentials.get(scheme_id)
            scheme = schemes.get(scheme_id, {})
            if not credential or not isinstance(scheme, dict):
                continue
            http_auth = scheme.get("httpAuthSecurityScheme")
            if not (isinstance(http_auth, dict) and http_auth):
                if isinstance(scheme.get("scheme"), str) and scheme["scheme"].strip():
                    http_auth = scheme
            if http_auth:
            ...
            if not (isinstance(api_key, dict) and api_key):
                if scheme.get("name") and scheme.get("location"):
                    api_key = scheme
            if api_key:
                loc = (api_key.get("location") or "").lower()
