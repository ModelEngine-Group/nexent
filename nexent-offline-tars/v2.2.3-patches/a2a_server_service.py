...
class A2AServerService:
    ...
    async def handle_message_stream(
        ...
        try:
            ...
            def _esc(s):
                # Escape a fragment for safe inline JSON streaming.
            ...
            async for chunk in stream_response.body_iterator:
                ...
                # Wrap thinking content in a JSON code block so frontends can
                # tell it apart from the final answer. Final answer is raw text.
                if chunk_type == "model_output_thinking":
                    escaped = _esc(text)
                    if not _in_thinking:
                        out = '```json\n{"thinking_content":"' + escaped
                        _in_thinking = True
                    else:
                        out = escaped
                    accumulated_text += out
                    yield self.adapter.build_a2a_task_event(
                        task_id=task_id,
                        event_type="taskProgress",
                        data={"content": out, "lastChunk": False},
                        context_id=context_id
                    )
                    ...
                    if _in_thinking:
                        ...
                        yield self.adapter.build_a2a_task_event(
                            task_id=task_id,
                            event_type="taskProgress",
                            data={"content": closing, "lastChunk": False},
                            context_id=context_id
                        )
                        _in_thinking = False
            ...
            # Close a still-open thinking block at end of stream.
            if _in_thinking:
                closing = '"}\n```'
                accumulated_text += closing
                yield self.adapter.build_a2a_task_event(
            ...
            # Final marker: do not resend full text (already streamed) to avoid garble.
            yield self.adapter.build_a2a_task_event(
                task_id=task_id or "simple",
                event_type="taskProgress",
                data={"content": "", "lastChunk": True},
                context_id=context_id
            )
