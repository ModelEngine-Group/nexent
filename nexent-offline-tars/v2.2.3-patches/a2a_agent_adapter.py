...
class A2AAgentAdapter:
    ...
    def extract_stream_chunk(self, chunk: Dict[str, Any]) -> Optional[str]:
        """Extract content from internal stream chunk for A2A consumers.

        Only final_answer and model_output_thinking are forwarded; all other
        chunk types are dropped to reduce A2A stream noise.

        Args:
            chunk: Internal stream chunk.

        Returns:
            Text to forward, or None to drop the chunk.
        """
        chunk_type = chunk.get("type", "")

        if chunk_type == "final_answer":
            return chunk.get("content") or None
