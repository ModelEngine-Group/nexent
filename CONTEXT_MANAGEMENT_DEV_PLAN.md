...
6. 在 `sdk/nexent/core/agents/context/policy_models.py` 中扩展 `MemoryPolicy` 和 `ContextPolicy` frozen dataclass
...
**验收标准**：
- [ ] `resolve_memory_levels()` 函数覆盖所有现有场景
- [ ] `store_memory_tool`、`search_memory_tool`、`create_agent_info.py` 三处重复逻辑已消除
- [ ] 现有 memory 相关测试全部通过（行为不变）
- [ ] `ContextPolicy` 和 `MemoryPolicy` 是 frozen dataclass
- [ ] `resolve_policy()` 正确合并各层配置
- [ ] 非法策略（预算超限、层级矛盾）在解析阶段被拒绝
- [ ] 单元测试覆盖所有 level 过滤分支 + 合并 precedence + 验证分支
...
- [ ] `SearchMemoryTool`、`StoreMemoryTool`、被动注入三条路径都通过 `decide_memory_operation()`
- [ ] 默认策略下行为与重构前一致（零回归）
- [ ] 策略引擎的 deny 决策正确阻止 memory 操作
- [ ] 单元测试覆盖所有选择分支 + allow/deny/confirm-required 决策
...
**验收标准**：
- [ ] 4 个 handler 的 `reduce()` 从 passthrough 升级为语义实现
...
| 阶段 | 可并行的 PR | 说明 |
|------|------------|------|
| Phase 0 完成后 | PR-1 ∥ PR-3 | W12 组件投影和 W13 Memory 统一无依赖 |
| Phase 1 中期 | PR-2 ∥ PR-4 | W12 DB 历史投影和 W13 选择引擎可并行 |
...
| W13 | 2 | Memory 统一 + Policy 模型 + 选择引擎 + Memory 决策 |
| W8 | 2 | 准入验证 + 确定性降级 + 语义降级 + 集成 |
| **合计** | **7** | |
...
| Memory 逻辑重构引入 bug | memory 读写异常 | PR-3 中保持接口不变，仅内部重构；全量回归测试 |
