...
const selectorPath = new URL(
  "../app/[locale]/agents/[agentId]/agent-selector-header.tsx",
  import.meta.url
);
...
test("uses URL synchronization as the only selector loading path", async () => {
  ...
  assert.match(
    selector,
    /requestedAgentIdRef\.current\s*=\s*requestedAgentId;\s*if\s*\(currentAgentId\s*!==\s*requestedAgentId\)\s*\{\s*void loadAgent\(requestedAgentId\);\s*\}/
  );
});
