...
test("treats a status without a terminal outcome as still processing", () => {
...
test("keeps every upload stage out of the terminal set", () => {
  assert.deepEqual(AIDP_DOC_IN_PROGRESS_STATUSES, [
    "UPLOADING",
    "PROCESSING",
    "EXTRACTING",
  ]);
  for (const status of AIDP_DOC_IN_PROGRESS_STATUSES) {
    assert.equal(
      isAidpDocTerminal(status),
      false,
      `${status} must not be terminal`
    );
    assert.equal(
      isAidpDocProcessing(status),
      true,
      `${status} must count as processing`
    );
  }
  // Case-insensitive, like every other status comparison.
  assert.equal(isAidpDocProcessing("extracting"), true);
  assert.equal(isAidpDocProcessing(" uploading "), true);
});
