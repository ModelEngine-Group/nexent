...
const getModelData = (t: any) => ({
  ...
  multimodal: {
    ...
    options: [
      { id: MODEL_TYPES.IMAGE_UNDERSTANDING, name: t("modelConfig.option.imageUnderstandingModel") },
      { id: MODEL_TYPES.IMAGE_GENERATION, name: t("modelConfig.option.imageGenerationModel") },
      { id: MODEL_TYPES.VIDEO_UNDERSTANDING, name: t("modelConfig.option.videoUnderstandingModel") },
    ],
  },
  voice: {
...
>((props, ref): ReactNode => {
  ...
  const loadModelLists = async (skipVerify: boolean = false) => {
    ...
    try {
      ...
      const imageUnderstandingExists = imageUnderstanding
        ? allModels.some(
            (m) => m.displayName === imageUnderstanding && m.type === MODEL_TYPES.IMAGE_UNDERSTANDING
          )
        : true;
      ...
      const imageGenerationExists = imageGeneration
        ? allModels.some(
            (m) => m.displayName === imageGeneration && m.type === MODEL_TYPES.IMAGE_GENERATION
          )
        : true;
      ...
      const videoUnderstandingExists = videoUnderstanding
        ? allModels.some(
            (m) => m.displayName === videoUnderstanding && m.type === MODEL_TYPES.VIDEO_UNDERSTANDING
          )
        : true;
  ...
  const verifyModelsInternal = async (
    ...
    if (!hasSelectedModels) {
      ...
      if (hasSelectedModels) {
        ...
        currentSelectedModels.multimodal.image_understanding = modelConfig.imageUnderstanding?.modelName || "";
        currentSelectedModels.multimodal.image_generation = modelConfig.imageGeneration?.modelName || "";
        currentSelectedModels.multimodal.video_understanding = modelConfig.videoUnderstanding?.modelName || "";
      ...
      for (const [category, options] of Object.entries(currentSelectedModels)) {
        for (const [optionId, modelName] of Object.entries(options)) {
          ...
          if (category === "voice") {
            ...
            // Map multimodal optionId to model type
            if (optionId === MODEL_TYPES.IMAGE_UNDERSTANDING) {
              modelType = MODEL_TYPES.IMAGE_UNDERSTANDING;
            } else if (optionId === MODEL_TYPES.IMAGE_GENERATION) {
              modelType = MODEL_TYPES.IMAGE_GENERATION;
            } else if (optionId === MODEL_TYPES.VIDEO_UNDERSTANDING) {
              modelType = MODEL_TYPES.VIDEO_UNDERSTANDING;
            } else {
  ...
  const applyModelChange = async (
    ...
    if (
      ...
      // Map multimodal optionId to config key
      if (option === MODEL_TYPES.IMAGE_UNDERSTANDING) {
        configKey = "imageUnderstanding";
      } else if (option === MODEL_TYPES.IMAGE_GENERATION) {
        configKey = "imageGeneration";
      } else if (option === MODEL_TYPES.VIDEO_UNDERSTANDING) {
        configKey = "videoUnderstanding";
      } else {
                        ...
                        type={
                          key === "voice"
                            ? option.id === MODEL_TYPES.TTS
                              ...
                              : MODEL_TYPES.STT
                            : key === "multimodal"
                              ? option.id as ModelType  // Use dynamic option.id for multimodal types
