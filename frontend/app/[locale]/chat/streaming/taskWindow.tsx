...
const messageHandlers: MessageHandler[] = [
  {
    canHandle: (message) =>
      message.type === chatConfig.messageTypes.CONTEXT_BUDGET,
    render: (message, t) => {
      try {
        const budget = JSON.parse(message.content || "{}");
        const raw = Number(budget.raw_tokens || 0);
        const final = Number(budget.final_tokens || 0);
        const saved = Number(budget.compression?.saved_tokens || 0);
        const ratio = Number(budget.compression?.ratio || 0);
        const reasons = Array.isArray(budget.compression?.reasons)
          ? budget.compression.reasons.filter(
        ...
        return (
          <div className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
            <div className="flex items-center gap-2 font-medium">
              ...
              {t("taskWindow.contextBudget.optimized")}
            </div>
            <div className="mt-1 text-xs text-blue-800">
              {t("taskWindow.contextBudget.savings", {
            ...
            {reasons.length > 0 && (
              <div className="mt-1 text-xs text-blue-800">
                {t("taskWindow.contextBudget.reason", {
                  reason: reasons
                    .map((reason: string) =>
                      t(`taskWindow.contextBudget.reasons.${reason}`, reason)
                    )
                    .join(", "),
                })}
              </div>
