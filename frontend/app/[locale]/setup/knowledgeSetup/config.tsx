...
function DataConfig({ isActive }: DataConfigProps) {
  ...
  useLayoutEffect(() => {
    ...
    if ((prevIsActive === null || !prevIsActive) && isActive) {
      ...
      hasUserInteractedRef.current = false; // Reset interaction state, prevent accidental saving
    }

    // Save user configuration when leaving second page
    if (prevIsActive === true && !isActive) {
      // Only save if user has interacted, prevent saving empty state during initial load
  ...
  // Save current state to ref in real-time, ensure it can be accessed during unmount
  useEffect(() => {
...
const getAuthHeaders = () => {
  ...
  useEffect(() => {
    return () => {
      // When component unmounts, if it was previously active and user has interacted, execute save
      if (prevIsActiveRef.current === true && hasUserInteractedRef.current) {
        ...
        try {
          // Use fetch with keepalive to ensure request can be sent when page unloads
          fetch(API_ENDPOINTS.tenantConfig.updateKnowledgeList, {
            ...
            console.error('Failed to save during unmount:', error);
          });
        } catch (error) {
          console.error('Save request exception during unmount:', error);
        }
      }
    };
  }, []);

  // Separately monitor knowledge base loading state, load user configuration when knowledge base loading is complete and in active state
  useEffect(() => {
  ...
  const generateUniqueKbName = (existingKbs: KnowledgeBase[]): string => {
    ...
    // Otherwise try adding numeric suffix until finding an unused name
    let counter = 1;
  ...
  const handleKnowledgeBaseClick = (kb: KnowledgeBase, fromUserClick: boolean = true) => {
    // Only reset creation mode when it's a user click
    if (fromUserClick) {
    ...
    // Call knowledge base change handler
  ...
  const handleKnowledgeBaseChange = async (kb: KnowledgeBase) => {
    try {
      ...
      // Update knowledge base statistics in background, but don't duplicate document fetching
      setTimeout(async () => {
        try {
          // Directly call fetchKnowledgeBases to update knowledge base list data
          await fetchKnowledgeBases(false, true);
        } catch (error) {
          console.error("Failed to get latest knowledge base data:", error);
        }
      }, 100);
    } catch (error) {
  ...
  const handleCreateNew = () => {
    ...
    setHasClickedUpload(false); // Reset upload button click state
    setUploadFiles([]); // Reset upload files array, clear all files to be uploaded
  };
  ...
  const viewingDocuments = (() => {
    // Return empty array in creation mode because new knowledge base doesn't have documents yet
    if (isCreatingMode) {
      return [];
    }
  ...
  // As soon as documents are uploaded successfully, immediately switch creation mode to false
  useEffect(() => {
  ...
  // In creation mode, when knowledge base name changes, reset "name already exists" state
