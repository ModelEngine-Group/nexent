          ...
          <div className="px-6 py-5 space-y-4">
            ...
            {websiteUrl || repositoryUrl ? (
              <div className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm text-slate-700">
                ...
                {repositoryUrl ? (
                  <div className="flex flex-wrap gap-2">
                    <span className="text-slate-500">{t("mcpTools.market.repository")}</span>
                    <a
                      href={repositoryUrl}
                      target="_blank"
                      rel="noreferrer"
            ...
            {service.packages.length > 0 ? (
              <div className="space-y-2">
                <p className="text-sm font-semibold text-slate-900">{t("mcpTools.market.packages")}</p>
                <div className="space-y-2">
                  {service.packages.map((pkg, index) => (
                    <div key={`${service.name}-${pkg.registryType}-${pkg.identifier}-${pkg.version}-${index}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                      ...
                      {pkg.transport?.url ? (
                        <p className="break-all text-slate-600">{pkg.transport.type || t("mcpTools.market.remoteFallback")}: {pkg.transport.url}</p>
                      ) : null}
                    </div>
                  ))}
                </div>
