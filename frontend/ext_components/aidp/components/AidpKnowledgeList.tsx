...
interface AidpKnowledgeListProps {
   ...
   *  filtering itself happens server-side: AIDP narrows the catalog by this
   *  keyword, so the page below renders exactly what the API returned and the
   *  reported total always matches the rendered cards. */
  keyword: string;
...
const AidpKnowledgeList: React.FC<AidpKnowledgeListProps> = ({
  ...
  // The keyword filter lives on the server: the parent forwards it to AIDP, and
  // filtering the returned page again here would hide results whenever AIDP's
  // matching is not a plain substring of the name/description. Only the order is
  // local, so the most recently updated KB stays on top of the page.
