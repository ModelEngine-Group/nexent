...
export const DEFAULT_ERROR_MESSAGES: Record<number, string> = {
  ...
  [ErrorCode.STT_CONNECTION_FAILED]:
    "Failed to connect to speech recognition service.",
  [ErrorCode.TTS_CONNECTION_FAILED]:
    "Failed to connect to speech synthesis service.",
...
export const getErrorMessage = (code: number): string => {
  return (
    DEFAULT_ERROR_MESSAGES[code] || "An error occurred. Please try again later."
  );
};
