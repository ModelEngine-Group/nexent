...
工具搜索可以早于最终信息收集完成。搜索结果可帮助后续澄清，但最终确认卡只能在至少完成一次工具搜索后生成。
...
- `ready_to_create`：当草稿已经足够完整且至少完成一次工具搜索时，调用 `present_creation_confirmation_card`。不得仅通过文本宣称创建完成。
...
`search_installed_mcp_tools` 接收当前生成的 `GeneratedAgentDraft`，服务端从草稿字段构造检索文本，并通过 `ProcessType.CARD` 直接发送工具推荐卡。

`present_creation_confirmation_card` 接收当前完整草稿和最近一次有效工具搜索的标识，通过 `ProcessType.CARD` 发送最终确认卡。没有有效工具搜索结果时拒绝调用。
...
没有匹配结果时发送空状态工具卡，并允许用户不绑定工具完成创建。
...
- 新的工具搜索结果会使旧工具推荐卡和基于旧修订生成的确认卡变为 `superseded`。
...
4. 以租户、用户和 `card_id` 作为幂等键查询已有创建结果。
5. 在单个事务中创建目标智能体 `version_no=0` 草稿并写入工具绑定。
6. 保存幂等键与 `target_agent_id` 的对应结果。
...
相同幂等键重复提交时返回已创建的草稿。校验失败或事务失败时不得保留部分智能体、工具绑定或成功幂等记录；卡片标记为 `failed`，瞬时错误允许重试。
...
- 最终确认工具在没有有效工具搜索时拒绝生成卡片。
- 确认接口正确校验草稿、工具权限、正整数 ID 和稳定去重。
- 伪造、跨租户、非 MCP 和不可用工具均被拒绝。
- 草稿创建、工具绑定和幂等结果满足原子性。
- 网络超时后的重复确认返回同一个 `target_agent_id`。
