...
| `nexent-common.config.cas.renewEnabled` | `CAS_RENEW_ENABLED` | Enable automatic browser iframe renewal; default `"true"`, `"false"` disables it |
...
To disable automatic renewal, set `nexent-common.config.cas.renewEnabled` to the string `"false"`; when using the deployment script, set `CAS_RENEW_ENABLED=false` in `deploy/env/.env`. After upgrading the frontend and backend, apply the Helm configuration, wait for the backend rollout, and reload open browser pages. Restore `"true"` to enable renewal again. CAS login, activity heartbeat, renewal endpoints, and session expiry handling remain unchanged; `force` mode still redirects to login and reloads the page at expiry.
