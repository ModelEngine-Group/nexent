...
- 可移植：同一套埋点可以通过 OTLP 上报到 Phoenix、Langfuse、Grafana Tempo、SkyWalking 或其他兼容后端，切换平台主要调整配置和 Collector pipeline。
...
Nexent 的监控能力以 OpenTelemetry 为主干，SDK 和后端只负责生成标准 span、event、metric，并通过 OTLP 导出。Phoenix、Langfuse、Grafana Tempo、Apache SkyWalking 和标准 OTLP 后端作为可配置 exporter 接入，业务代码不绑定单一平台。
...
| 配置 | 环境变量 |
| Collector | `otel/opentelemetry-collector-contrib`，支持 logging、Phoenix、Langfuse、Grafana/Tempo、SkyWalking 本地部署形态 |
...
| `MONITORING_PROVIDER` | `otlp` | 监控 provider 和本地部署形态：`otlp`、`phoenix`、`langfuse`、`grafana`、`skywalking` |
...
与 Phoenix、Langfuse、Grafana Tempo、SkyWalking 对比：
