...
>
> 当前实现：Git commit `604151c6`
>
> 目的：按真实调用路径解释当前版本相对基线新增和修改的实现
...
相对 `lvnqqsqn`，当前快照涉及 106 个文件。代码可以分为十组：
...
工作区还删除了被跟踪的 `.claude/settings.local.json` 并更新 `.gitignore`；这与运行逻辑无关。
...
nl2agent_catalog_service.load_session_catalogs()
  ...
  ├─ Registry service(search=None, limit=30) → registry_mcps
  ├─ Community service(search=None, limit=30) → community_mcps
...
任一 provider 抛出异常会转换为 `Nl2AgentCatalogUnavailableError`。空列表本身不报错。
...
HTTP 请求体只提供 Skill ID/name；Backend 必须在当前 Draft Catalog 中解析到 installable 项。安装与把 SkillInstance 绑定到 Draft 是不同概念。当前 endpoint 和 `WebSkillCard` 只安装 Skill、更新 Card 状态并从 Session Catalog 删除推荐，不会为 Draft 创建 `SkillInstance`。
...
Parser 产生 `ValidatedNl2AgentCard` AST，其中含 payload、card type、card key 和 `requiresRegistration`。`nl2agent-search-*` 不在 tag 映射中。

Renderer 和 Final Message Delivery 都复用这套 parser/schema，但当前分别调用 `parseNl2AgentCard()` 与 `validateNl2AgentCards()`；一个调用生成的 AST 不会直接传给另一个调用。因此当前消除了多套 schema，却仍保留同一内容的两次解析。
...
7. Session Start response TypeScript interface 仍保留一个可选 `agent_id` alias，但当前入口使用明确 Runner/Draft 字段。
8. 部分 response/Session State TypeScript 类型手写，request 类型更多来自生成 OpenAPI。
9. 部分 Card 文案仍硬编码英文；阅读 i18n 文件时不要假设所有 label 都已迁移。
10. `backend/database/tool_db.py` 的旧注释“6 tools”与实际三个定义不一致。
11. 不是所有 NL2AGENT endpoint 的异常映射都已经统一为结构化领域错误。
12. Finalize 成功状态当前叫 `draft_ready`；代码没有把用户操作描述成自动发布后台任务。
13. online Skill Card 的“Installed”只代表租户级 Skill 安装成功；当前链路不创建 Draft `SkillInstance`，最终审核只会显示另外已经持久化绑定的 SkillInstance。
14. Card schema/parser 已统一，但 Renderer 与 Delivery effect 仍分别解析一次，没有共享同一个 AST 对象。
15. `Nl2AgentContext` 是实例级而非全局级，但没有通过 frozen/deep copy 强制不可变；后续新增工具逻辑时必须避免就地修改注入 Catalog。
