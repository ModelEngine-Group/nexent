...
- `nl2agent_catalog_service.py`：加载 Tool、Skill、Registry MCP、Community MCP 与官方 Skill Catalog，在写入 Session 前统一脱敏，并校验和安装可信在线 Skill 推荐。
- `nl2agent_workflow_service.py`：在线审查、Card Delivery、需求确认、Session State 投影与身份确认。
- `nl2agent_session_service.py`：验证默认 Builder、加载 Catalog、事务创建 draft/Conversation，并补偿 Redis/数据库初始化失败。
- `nl2agent_mcp_service.py`：稳定 installation key、跨 worker 安装锁、远程/容器安装、工具发现恢复、Tool provenance 校验、绑定与显式跳过。
- `nl2agent_service.py`：兼容 facade，负责组装上述 Service 的依赖，并暂时保留默认 Agent seed 和模型/资源展示投影辅助逻辑。
