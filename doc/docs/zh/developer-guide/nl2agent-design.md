...
PostgreSQL 是会话、不可变目录和工作流的唯一权威存储，工作流修改使用严格递增的 revision CAS。安装由 `nl2agent_installation_operation_t` 持久化：请求指纹、checkpoint、attempt、lease owner/expiry、结果引用和脱敏错误都在数据库中保存；外部 I/O 不持有数据库连接。完成 operation 可幂等重放，存活 lease 冲突，过期 lease 可接管。Redis 不参与 NL2Agent 状态、目录或安装恢复。
...
- `Nl2AgentWorkflowProvider` 是唯一 Session-scoped coordinator，集中 scope 校验、卡片注册与 delivery、输入锁、隐藏续聊、失败重试、operation 状态刷新和 session state 恢复。
...
Session start 在同一事务创建 Draft Agent、内部 conversation 和 PostgreSQL session。启动时只做有界的过期 operation lease 回收；旧 session 不恢复 Builder conversation，已有 Draft Agent 仍保留。
