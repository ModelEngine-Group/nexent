...
> **注意**：删除对话后通常无法恢复，操作前请确认。

![智能体管理](./assets/start-chat/conversation-manage.png)
...
当回复内容使用了检索结果时，相关句子的末尾会显示圆形引用编号（如 `1`、`4`）；同一句同时参考多个来源时，会连续显示多个编号。

- **鼠标悬停**引用编号：显示摘要卡片，包含来源标题、命中文本片段和来源信息。
- **点击**引用编号：右侧来源面板自动定位到对应的来源卡片，并展开本次实际命中的文本片段（Chunk）。
...
- **查看文档内容**：在对话区域直接预览生成的文档
- **检查生成效果**：确认文档内容是否符合需求
- **继续修改**：要求智能体对文档进行修改或补充
- **下载文档**：点击下载按钮保存到本地

![文档生成](./assets/start-chat/preview-docx.png)
