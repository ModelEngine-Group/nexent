...
本页面说明 Nexent 平台的角色体系、数据可见范围和资源操作权限，并介绍 API Key、知识库容量及常用资源筛选方式。

⚠️ **重要提示**：首次部署 v1.8.0 及以上版本时，系统会创建 `suadmin@nexent.com` 超级管理员账号，默认密码为 `Nexent@123`，创建成功后会在终端显示。可在首次部署前通过 `NEXENT_SUPER_ADMIN_PASSWORD` 覆盖；离线部署包使用 `--config` 时则以交互输入的密码为准，且不会显示手动输入的密码。
...
- [四、权限配置](#四权限配置) - 智能体、知识库、API Key 与容量管理
...
| 角色           | 职责描述                                       | 适用场景             | 角色备注                                                                       |
| -------------- | ---------------------------------------------- | -------------------- | ------------------------------------------------------------------------------ |
| **超级管理员** | 可创建**不同租户**，管理所有租户资源           | 平台运维人员         | Nexent 系统只有一个超级管理员，首次部署默认创建，可通过部署环境变量预设密码    |
...
| **普通用户**   | 使用已发布智能体，并维护自己的私有知识库       | 员工、业务人员       | 同一租户下可拥有多个普通用户，可属于租户下多个用户组，由管理员和超级管理员邀请 |
...
普通用户以使用已发布智能体为主，也可以创建和维护自己的私有知识库。

- ✅ 可以使用被授权的智能体进行对话
- ✅ 可以创建、编辑和删除自己的私有知识库，并查看个人容量
- ✅ 可以查看获得授权的共享知识库
- ✅ 可以查看自己的使用记录和个人信息
- ❌ 不能创建或编辑智能体，也不能把个人知识库改为共享知识库
...
| 操作                     | 超级管理员 | 管理员 |       开发者       | 普通用户 |
| ------------------------ | :--------: | :----: | :----------------: | :------: |
| **查看知识库列表**       |     ✅     |   ✅   | 🟡 自己创建/被授权 | 🟡 个人/被授权 |
| **查看知识库详情**       |     ❌     |   ✅   | 🟡 自己创建/被授权 | 🟡 个人/被授权 |
| **查看知识库总结**       |     ✅     |   ✅   | 🟡 自己创建/被授权 | 🟡 个人/被授权 |
| **创建知识库**           |     ❌     |   ✅   |         ✅         | 🟡 仅私有知识库 |
| **编辑知识库名称和权限** |     ✅     |   ✅   | 🟡 自己创建/被授权 | 🟡 仅自己的私有知识库，不能改为共享 |
| **编辑知识库分块、总结** |     ❌     |   ✅   | 🟡 自己创建/被授权 | 🟡 仅自己的私有知识库 |
| **删除知识库**           |     ✅     |   ✅   | 🟡 自己创建/被授权 | 🟡 仅自己的私有知识库 |
| **上传/删除文件**        |     ❌     |   ✅   | 🟡 自己创建/被授权 | 🟡 仅自己的私有知识库 |
...
普通用户创建的知识库固定为“私有”，不能添加用户组，也不能改为共享知识库。管理员和开发者创建知识库时，可以根据协作范围设置用户组和组内权限。

### 4.3 API Key 管理

租户管理员可以进入“租户资源”中的“API Key”页签，集中查看当前租户的接口凭证。列表包含 API Key、所属用户、角色、创建人、创建时间、最近使用时间和累计使用次数；没有对应邮箱的记录会标记为“虚拟接口账户”。

支持以下管理操作：

- **刷新**：使该用户原有的有效 API Key 失效，并生成新 Key。新值只在成功弹窗中显示，应立即复制到安全的凭证系统。
- **删除**：撤销该用户的全部有效 API Key。依赖这些凭证的外部调用会立即受到影响。
...
超级管理员和租户管理员可以在“租户资源”→“知识库”中切换“共享知识库 / 个人知识库”视图。个人知识库视图提供：

- 使用个人知识库的用户数、个人知识库总数、总用量和已分配总配额
- 按用户名或邮箱搜索，按用量、配额等字段排序和分页
- 展开用户后查看其每个个人知识库的文档数、Chunk 数和占用空间
...
资源列表优先使用页面提供的搜索与筛选条件，不需要逐页查找：

- 用户列表可按用户名、邮箱和角色定位用户。
- 用户组选择器支持输入名称搜索。
- 知识库列表可按名称、用户组等条件筛选；个人容量列表可按用户搜索。
- 智能体、MCP 和 Skill 列表可结合名称、状态、标签或所属范围筛选，具体选项以当前页为准。

筛选条件只缩小当前账号本来就有权查看的资源范围，不会扩大权限或显示其他租户的数据。
